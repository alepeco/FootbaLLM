...
def collect_team_data_LaLiga(match_logs_url, team_name, max_retries=3):
    headers = {
...
def collect_player_data(team_url, headers, max_retries=3):
    player_data = []
    for attempt in range(max_retries):
        try:
            ...
            player_rows = soup.select('#stats_standard_12 tr[data-row]')

            for row in player_rows:
                player_name = row.find('th', {'data-stat': 'player'}).text.strip()
                player_href = row.find('th', {'data-stat': 'player'}).find('a')['href']
                player_url = f"https://fbref.com{player_href}"

                # Scrape player page
                for attempt_player in range(max_retries):
                    try:
                        time.sleep(random.uniform(5, 10))  # Delay to avoid rate-limiting
                        player_response = requests.get(player_url, headers=headers)
                        if player_response.status_code != 200:
                            raise Exception(f"HTTP {player_response.status_code}: {player_response.reason}")

                        player_soup = BeautifulSoup(player_response.text, 'html.parser')
                        last_5_table = pd.read_html(player_response.text, attrs={'id': 'last_5_matchlogs'})[0]
                        last_5_table['name'] = player_name  # Add player name column
                        player_data.append(last_5_table)
                        break  # Exit retry loop for player page if successful
                    except Exception as e:
                        print(f"⚠️ Failed to fetch data for player {player_name}: {e}")
                        if attempt_player == max_retries - 1:
                            print(f"❌ Skipping player {player_name} after {max_retries} attempts.")
                        time.sleep(10)
            break  # Exit retry loop for team page if successful
        except Exception as e:
            print(f"⚠️ Attempt {attempt + 1} failed for team page {team_url}: {e}")
            if attempt == max_retries - 1:
                raise  # re-raise error if last attempt
            print("🔁 Retrying in 10 seconds...")
            time.sleep(10)

    return pd.concat(player_data, ignore_index=True) if player_data else pd.DataFrame()
...
for url, team in teams_info:
    try:
        ...
        df = collect_team_data_LaLiga(url, team)
...
for url, team in teams_info:
    try:
        print(f"Fetching player data for {team}...")
        player_df = collect_player_data(url, headers)
        player_dataframes.append(player_df)
    except Exception as e:
        print(f"❌ Failed to collect player data for {team}: {e}")

# Combine all player data
if player_dataframes:
    combined_player_df = pd.concat(player_dataframes, ignore_index=True)
    os.makedirs('data', exist_ok=True)
    combined_player_df.to_csv('data/Player_Data.csv', index=False)
    print("Player data CSV saved at data/Player_Data.csv")
else:
